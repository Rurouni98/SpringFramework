            ...
            <div class="container mt-3 mb-5 w-75 card-wrapper">
                <c:forEach items="${noticeBoardList}" var="noticeBoard">
                    <div class="card" style="width: 18rem;">
                        ...
                        <div class="card-body">
                            <h5 class="card-title">${noticeBoard.title}</h5>
                            <p class="card-text">작성일:
                                <javatime:format value="${noticeBoard.regdate}" pattern="yyyy-MM-dd"/>
                            </p>
                            <a href="/board/update-cnt.do?id=${noticeBoard.id}&type=notice" class="btn btn-outline-secondary btn-sm">자세히 보기</a>
                        </div>
                    </div>
                </c:forEach>
            </div>
    ...
    <script>
        $(() => {
            ...
            $("input[name=searchKeyword]").on("keypress", (e) => {
                if(e.key === 'Enter') {
                    $("input[name='pageNum']").val(1); // 엔터로 검색 시 pageNum을 1로 초기화
                }
            });
        });
    </script>
